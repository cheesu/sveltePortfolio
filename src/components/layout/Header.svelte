...
<header id="headerNav" class="fixed top-0 w-full text-white z-10">
	<div class="mx-auto py-4 flex justify-between items-center mx-aut w-full  sm: lg: s-7IPF32Wcq3s8">
		<a id="headerLogo" href="/" class="text-2xl font-nanumb font-bold w-120 mx-5 md:mx-10"
			>In <span class="text-red-500">S</span>tudio</a
		>
		<button class="mr-5 md:hidden" on:click={handleClick}>
			<svg
				id="bugerBar"
		...
		<nav
			id="topNav"
			class={`transform transition-transform duration-500  fixed bg-dark h-full w-full top-0  text-4xl  mx-auto space-x-4  md:text-xl md:w-auto md:static md:bg-transparent ${
				$isMobileMenuOpen
			...
			<ul class="text-center md:flex">
				{#each menuItems as item}
					<li class="mt-7 pl-4 pr-4 md:pl-4 pr-4 md:mt-0">
						<a on:click={mobileMenuClose} href={item.url} class="font-bold hover:text-red-500 "
							>{item.title}</a
						>
					</li>
				{/each}
